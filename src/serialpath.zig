...
/// The path template is `/tmp/kvmgui-serial-{vm_name}.sock`.
pub fn serialSocketPath(buf: []u8, vm_name: []const u8) ![:0]const u8 {
    return std.fmt.bufPrintZ(buf, "/tmp/kvmgui-serial-{s}.sock", .{vm_name});
}
...
test "serialSocketPath: short name" {
    ...
    try std.testing.expectEqualStrings("/tmp/kvmgui-serial-test-vm.sock", path);
}

test "serialSocketPath: empty name" {
    var buf: [320]u8 = undefined;
    const path = try serialSocketPath(&buf, "");
    try std.testing.expectEqualStrings("/tmp/kvmgui-serial-.sock", path);
}

test "serialSocketPath: name with special chars" {
    var buf: [320]u8 = undefined;
    const path = try serialSocketPath(&buf, "my vm (copy)");
    try std.testing.expectEqualStrings("/tmp/kvmgui-serial-my vm (copy).sock", path);
}
...
test "serialSocketPath: exact fit" {
    ...
    // "/tmp/kvmgui-serial-" = 19 chars, ".sock" = 5 chars → 24 overhead
...
test "serialSocketPath: consistent format" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, path, "/tmp/kvmgui-serial-"));
